...
<script type="text/javascript" src="../js/table/treeTable.js"></script>
    ...
    function sousuo(){
      ...
      top.window.frmright.location.href="readLabelCustomXZSouSuo.action?sousuo="+sousuo1;
    }
    function hulue(id){
    top.Dialog.confirm("是否确定要忽略？",function(){
      $.post(
     "labelCustomHL.action",
     {"id":id},
     function(data){
       if(data.ok==false){
...
<table class="treeTable">
	...
	<tr id="node-${la.id }" class="child-of-node-${la.parentId }">
		...
		<s:if test="#la.parentId==0">
		   ...
		   <s:else>
		<input type="button" value="忽略" onclick="hulue(${la.id})"/>
